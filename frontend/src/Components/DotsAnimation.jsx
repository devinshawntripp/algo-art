...
const roundedCircleWave = (t, delta, a, f) => {
  return ((2 * a) / Math.PI) * Math.sinh(Math.sin(2 * Math.PI * t * f) / (delta * ((Math.PI) + .75)))
}
...
function Dots() {
  ...
  const { vec, transform, positions, distances } = useMemo(() => {
    ...
    const distances = positions.map((pos) => {
      ...
      return pos.length() + Math.cos(pos.angleTo(right) * 88) * 25
    })
  ...
  useFrame(({ clock }) => {
    for (let i = 0; i < 10000; ++i) {
      ...
      const wave = roundedCircleWave(t, 0.15 + (0.2 * dist) / 72, 0.4, 1 / 3.8)
